...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  const [events, rules, translations, scans, dsars, settings, sessions] =
    await db.$transaction([
      ...
      db.dataRequest.deleteMany({ where: { shop } }),
      db.shopSettings.deleteMany({ where: { shop } }),
      db.session.deleteMany({ where: { shop } }),
    ]);
  console.log(
    `Received ${topic} webhook for ${shop}: purged ${events.count} consent events, ${rules.count} region rules, ${translations.count} translations, ${scans.count} scan results, ${dsars.count} data requests, ${settings.count} settings rows, ${sessions.count} sessions`,
  );
